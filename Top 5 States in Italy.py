#Description: This program counts the top 5 states in ITALY causing the most number of injuries.
...
#Variable dict for storing the dictionary list
dict1 = collections.defaultdict(list)

#Getting top 5 places in ITALY with most number of injuries
maskcan =  (data2[:,5] == 'Italy')
#fetching data from csv file regarding locations in ITALY with most injuries in accident.col[4]
...
#Top 5 states in ITALY with most number of injuries.
...
plt.bar(y_position_2, top_5_can.values(), align='center', alpha=0.5, color='m')

plt.xticks(y_position_2, top_5_can.keys(), rotation=0)

plt.ylabel('FREQUENCY OF INJURY/STATE')

plt.title('TOP 5 STATES IN ITALY WITH MAX INJURIES')

#plt.savefig('INJURY FREQUENCY VS STATE:Italy.png', bbox_inches='tight')
